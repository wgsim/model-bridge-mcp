...
def test_save_if_requested_skips_body_file_on_failure(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    response = (
    ...
    assert not (tmp_path / ".model_bridge" / "outputs" / target).exists()
    meta_files = list(debug_dir.glob("*.meta.log"))
    assert len(meta_files) == 1
    assert "[FILE SKIPPED] No model body extracted from response." in out


def test_save_if_requested_masks_sensitive_text_in_meta(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    response = (
        "body\n\n--- [Routing Log] ---\n"
    ...
    _save_if_requested(response, target, tool_name="ask_chatgpt_cli", debug_dir=str(debug_dir))
...
def test_save_to_file_rejects_output_root_prefixed_relative_path(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)

    out = save_to_file("hello", ".model_bridge/outputs/result.txt")

    assert out.startswith("[SECURITY ERROR]")
    assert not (tmp_path / ".model_bridge" / "outputs" / "result.txt").exists()
