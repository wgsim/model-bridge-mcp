...
from model_bridge.core.response import (  # noqa: F401 – re-exported for back-compat
    DEBUG_META_DIR,
...
async def ask_chatgpt_cli(
    ...
    Args:
        ...
        save_path: Relative path such as 'reports/result.txt'; it is saved under .model_bridge/outputs/... and must not include the '.model_bridge/outputs/' prefix. Absolute paths and traversal escapes are rejected. If you previously passed an absolute path, migrate to a path relative to .model_bridge/outputs.
...
async def ask_batch(
    ...
    Args:
        ...
        save_path: Relative path such as 'reports/result.txt' forwarded to each ask() call; it is saved under .model_bridge/outputs/... and must not include the '.model_bridge/outputs/' prefix. Absolute paths and traversal escapes are rejected. If you previously passed an absolute path, migrate to a path relative to .model_bridge/outputs. The same relative path is forwarded to every inner ask() call, so later saves overwrite earlier ones and parallel mode is last-writer-wins.
        force_model: If True, skip failover chain.
