...
If that is not practical, use an equivalent wrapper such as `conda run --no-capture-output -n model-bridge-mcp_dev model-bridge` (or `--live-stream`) or an absolute interpreter/script path in the client configuration.
...
- Common options across ask tools:
  ...
  - `save_path`: pass a relative path like `reports/result.txt`; it is saved under `.model_bridge/outputs/...`; do not include the `.model_bridge/outputs` prefix; absolute paths and traversal escapes are rejected
